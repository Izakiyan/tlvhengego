app.factory('serviceHenge', ['$http', function($http) {
    ...
    var merge = function() {
        for (var i = 0; i < hengeStreet.length; i++) {
            for (var j = 0; j < hengeMoment.length; j++) {
                if ((hengeStreet[i].streetAngle === Math.round(hengeMoment[j].azimuth * 10) / 10) && new Date(hengeMoment[j].time).getFullYear() === new Date().getFullYear()) {
    ...
    var sort = function() {
        ...
        var temp = final.sort(function(a, b) {
